...
namespace Hybel.HealthSystem
{
    ...
    internal readonly struct Option<T> : IEquatable<Option<T>>
    {
        ...
        public static Option<T> Some(T value) => value.Some();
    ...
    internal static class OptionExtensions
    {
        ...
        /// <para>
        /// This method should be avoided in production, but is useful when you just want to ignore the errors.
        /// This means that anywhere an <see cref="Unwrap{T}(Option{T})"/> is called, there is fallable code.
        /// Use <see cref="TryUnwrap{T}(Option{T}, out T)"/> for better safety.
        /// </para>
        /// </summary>
        ...
        /// <summary>
        /// Yields only the <see cref="Option{T}.Some(T)"/> variants in the <paramref name="collection"/> after unwrapping them.
        /// </summary>
        /// <param name="collection">Collection containing <see cref="Option{T}"/>s possibly with <see cref="Option{T}.None"/> variants in it.</param>
        /// <returns>New collection where no values can be null.</returns>
        public static IEnumerable<T> Flatten<T>(this IEnumerable<Option<T>> collection)
        {
